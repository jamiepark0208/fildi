...
- [2026-06-25](claude-2026-06-25.md) — Scoring pipeline + peer DB infrastructure complete; Stock DB admin grid built; FMP backfill partially run (13/158 tickers), blocked on daily API limit | build | next: Restart Replit server. Verify Stock DB tab loads with 13 rows + 145 NO DATA rows. Run backfill:peers tomorrow when FMP resets. Then: file-split options-scanner.tsx(1199L), stock-breakdown.tsx(1084L), portfolio.tsx(955L). Remove user-configurable scoring weights UI (ScoringPreferencesContext).
