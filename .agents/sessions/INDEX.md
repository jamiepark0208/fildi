...
- [2026-06-25](claude-2026-06-25.md) — Unblocked app: two crashes fixed (pe_ratio key missing in V2 metrics, sofr optional chaining); context fills fast due to large console log pastes — keep session scope narrow | build | next: 1) Enhance Stock DB tab: add Fundamental group label, Technical columns, per-ticker status icon (queued/in-progress/done)
